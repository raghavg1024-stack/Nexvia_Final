...
import { getCollaborationsData } from "./actions";

export const dynamic = "force-dynamic";

export default async function CollaborationsPage() {
  const { collaborations, membership } = await getCollaborationsData();
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-10 sm:px-6">
      ...
      <Reveal>
        ...
        <p className="mt-2 text-sm text-slate-400">Manage FDPs, workshops, guest lectures, innovation challenges, consultancy, and research projects with academic institutions.</p>
      </Reveal>
      ...
      <Suspense fallback={<div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3"><div className="rounded-2xl border border-line bg-card p-6 animate-pulse h-48"/><div className="rounded-2xl border border-line bg-card p-6 animate-pulse h-48"/><div className="rounded-2xl border border-line bg-card p-6 animate-pulse h-48"/></div>}>
        <CollaborationsClient initialCollaborations={collaborations} hasMembership={!!membership} />
      </Suspense>
    </main>
  );
}